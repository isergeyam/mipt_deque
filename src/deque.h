	...
	public:
														...
														_Ref>
		{
			...
			operator _Deque_iterator<const T&, const Deque<T>*>() const {
				return _Deque_iterator<const T&, const Deque<T>*>(const_cast<Deque<T>*>(start_), cur_) ;
			}
		...
		Deque(InputIterator begin, InputIterator end) :
			capacity_(2*std::distance(begin, end)),
			head_(0),
		...
		_Self& operator=(_Self cp) {
			std::swap(array_, cp.array_) ;
			head_=cp.head_ ;
			tail_=cp.tail_ ;
